...
const useScanner = (selectedProduct: any) => {
  ...
  const variance = totalScanned - (selectedProduct?.theoretical || 0);
  ...
  const handleScan = (lotNumber: string) => {
    ...
    if (!lot) {
      const otherProduct = MOCK_PRODUCTS.find(p =>
        p.id !== selectedProduct.id &&
        p.lots.some((l: any) => l.id === lotNumber)
      );

      setError(otherProduct
        ? `Lot ${lotNumber} belongs to "${otherProduct.name}"`
        : `Invalid lot: ${lotNumber}`
      );
...
export default function Scanner() {
  ...
  useEffect(() => {
    ...
    } else if (!selectedProduct && MOCK_PRODUCTS.length > 0) {
  ...
  const sortedLots = selectedProduct?.lots.sort((a: any, b: any) => {
  ...
  const stats = [
    { label: 'Expected', value: selectedProduct?.theoretical || 0, color: theme(isDarkMode, 'text-gray-300', 'text-gray-700') },
      ...
      <div className={isDesktop ? "p-4" : "p-4 pb-32"}>
        <div className={`rounded-lg p-4 ${
          ...
          <div className="space-y-2">
            {sortedLots.map((lot: any) => <LotItem key={lot.id} lot={lot} />)}
          </div>
        </div>
      </div>
